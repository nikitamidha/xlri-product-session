...
A **20-minute deep dive** on one product, structured around three questions:
...
| [`wispr-consumer-ladder.md`](wispr-consumer-ladder.md) | **The consumer ladder — the session artifact.** Eleven jobs for one consumer, built up from the ground floor (language became the interface) to paying for it. Four columns: the job in JTBD form, the feature that solves it, the pre-gen-AI context and the post-gen-AI context side by side. Enterprise is out of scope by design. |
| [`wispr-feature-jobs.md`](wispr-feature-jobs.md) | **The feature–job map.** Fourteen features, each run down five levels of *why* into the user's context, then written back out as a job with outcome statements. Adds the two buyer jobs the portfolio omits. |
