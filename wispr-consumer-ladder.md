...
| **2 — The feature** | The feature, or feature set, that solves it. Stack layers as numbered in [`wispr-flow-deep-dive.md`](wispr-flow-deep-dive.md) §4. |
...
**Why a ladder.** Each rung only becomes available once the one beneath it is
solved. Read top to bottom — it's the order a real user meets them in.
...
| --- | --- | --- | --- |
| **L1** | Describe the problem instead of operating the tool | Hold-to-talk + every text field, no length penalty | **CHANGED — new circumstance** |
| **L2** | Get to the right answer by going back and forth | Cheap repeat turns + AI Commands | **CHANGED — new circumstance** |
| **L3** | Do it in the places I actually am | Canto + hold-to-talk + phone apps | **CREATED BY ADOPTION** |
| — | *— from here on, it has to sound human —* | | |
| **L4** | Reply to people at the speed I'd say it | The cleanup layer | UNCHANGED — the last mile moved to the product |
| **L5** | Say it before I've fully worked it out | Self-correction resolution | UNCHANGED — the precondition is gone |
| **L6** | One way of speaking, right wherever it lands | Context-awareness | UNCHANGED — but no pre-AI equivalent exists |
| **L7** | Be read the way I sound when I speak | Cleanup used as a fluency layer | UNCHANGED — correction became rendering |
| **L8** | Think in one language, send in another | One-pass capture, cleanup, translation | UNCHANGED — three steps became one |
| **L9** | Be understood on my own vocabulary | Passive personalisation | UNCHANGED — setup moved after the value |
| **L10** | Be sure before I pay | Free tier metered in words, not features | **CHANGED — what you're buying is different** |
...
**1. The first three rungs built the market for the rest.** Talking to a machine
...
compound.** L1 and L2 grow with AI adoption. L4–L9 grow with population, which
moves a few percent a year.

**3. Six rungs are the same circumstance as 1995.** Nothing about the user's world
...
**5. Two rungs are the product's own footprints.** L3 and L10 could not have
...
**6. Speed is rarely the reason.** Most rungs are about being understood,
...
Put the glance table up, then take **three rungs**, one per block:

| Rung | Why this one |
| --- | --- |
| **L1** | The ground floor and the whole argument in one row. Ask the room how many sentences they typed *at a machine* last week versus in 2021. The answer is the market. |
| **L5** | The most human rung, and the best live demo. Say a sentence, change your mind halfway, watch the text keep only what you settled on. |
| **L10** | The pricing rung, and the one they can use on Monday: *is my free tier testing my capability, or letting a habit form?* |

**Then the exercise.** Give teams a consumer product and twenty minutes to build
the same four columns. Two tests:

- **Is their bottom rung actually the bottom?** Most teams start two rungs too
  high, at a job that only exists once something below it is solved.
